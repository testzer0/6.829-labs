...
class BBTopo(Topo):
    ...
    def __init__(self, n=2):
        ...
        self.addLink(h1,s0,1,1,bw=args.bw_host,max_queue_size=args.maxq,delay=args.delay)
        self.addLink(s0,h2,2,2,bw=args.bw_net,delay=args.delay)
...
def start_tcpprobe(outfile="cwnd.txt"):
    ...
    #      shell=True)
    # tcp_probe has been deprecated

    os.system("echo 1 > /sys/kernel/debug/tracing/events/tcp/tcp_probe/enable")

def stop_tcpprobe(outfile="cwnd.txt"):
    Popen("cat /sys/kernel/debug/tracing/trace > %s" % (outfile), shell=True)
    sleep(3)
...
def start_iperf(net):
    ...
    client = h1.popen("iperf -c "+h2.IP()+" -P 10")
...
def start_webserver(net):
    h2 = net.getNodeByName('h2')
    print("Starting webserver on h2...")
...
def start_ping(net, outfile="ping.txt"):
    ...
    h1 = net.getNodeByName('h1')
    h2 = net.getNodeByName('h2')
    h1.popen("ping "+h2.IP()+" -i 0.1 > "+outfile, shell=True)

def do_curl(net):
    h1 = net.getNodeByName('h1')
    h2 = net.getNodeByName('h2')
    h1.popen("curl -o /dev/null -s -w \%\{time_total\} "+h2.IP())
...
def bufferbloat():
    ...
    start_iperf(net)
    procs = start_webserver(net)
    start_ping(net, "%s/ping.txt" % (args.dir))
    ...
    stop_tcpprobe("%s/cwnd.txt" % (args.dir))
    qmon.terminate()
    net.stop()
