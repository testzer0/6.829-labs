...
def parse_file(f):
    ...
    for l in open(f).xreadlines():
        ...
        if not args.sport:
            # print("%s %s" % (fields[5].split(':')[1], args.port))
            if fields[5].split(':')[1] != args.port:
                continue
        else:
#            print "using sport %s (compare with %s)" % (args.port, fields[1].split(':')[1])
            if fields[6].split(':')[1] != args.port:
                continue
        sport = int(fields[5].split(':')[1])
        times[sport].append(float(fields[3][:-1]))

        c = int(fields[11].split('=')[1])
        cwnd[sport].append(c * 1480 / 1024.0)
        srtt.append(int(fields[14].split('=')[1]))
    return times, cwnd
